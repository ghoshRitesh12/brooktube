package constants

const (
	HOST                    string = "https://music.youtube.com"
	BROWSE_PATH             string = "/youtubei/v1/browse"
	PLAYLIST_PATH           string = "/playlist?list="
	PLAYER_PATH             string = "/youtubei/v1/player"
	NEXT_PATH               string = "/youtubei/v1/next"
	QUEUE_PATH              string = "/youtubei/v1/music/get_queue"
	SEARCH_PATH             string = "/youtubei/v1/search"
	SEARCH_SUGGESTIONS_PATH string = "/youtubei/v1/music/get_search_suggestions"
)

const (
	MUSIC_PAGE_TYPE_ALBUM              string = "MUSIC_PAGE_TYPE_ALBUM"
	MUSIC_PAGE_TYPE_ARTIST             string = "MUSIC_PAGE_TYPE_ARTIST"
	MUSIC_PAGE_TYPE_PLAYLIST           string = "MUSIC_PAGE_TYPE_PLAYLIST"
	MUSIC_PAGE_TYPE_USER_CHANNEL       string = "MUSIC_PAGE_TYPE_USER_CHANNEL"
	MUSIC_PAGE_TYPE_ARTIST_DISCOGRAPHY string = "MUSIC_PAGE_TYPE_ARTIST_DISCOGRAPHY"
	MUSIC_PAGE_TYPE_TRACK_LYRICS       string = "MUSIC_PAGE_TYPE_TRACK_LYRICS"
)

const MUSIC_EXPLICIT_BADGE string = "MUSIC_EXPLICIT_BADGE"
const MUSIC_ITEM_RENDERER_DISPLAY_POLICY_GREY_OUT string = "MUSIC_ITEM_RENDERER_DISPLAY_POLICY_GREY_OUT"

const USER_AGENT_HEADER string = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36"

const (
	HL             string = "en"
	PLATFORM       string = "DESKTOP"
	TIME_ZONE      string = "Europe/Berlin"
	CLIENT_NAME    string = "WEB_REMIX"
	CLIENT_VERSION string = "1.20240422.01.01"
)

const (
	GOOG_FIELD_MASK string = "*"
	GOOG_VISITOR_ID string = "CgtKc19vbFBzMEpYcyjfz6-xBjIKCgJJThIEGgAgJA%3D%3D"
	GOOG_API_KEY    string = "AIzaSyC9XL3ZjWddXya6X74dJoCTL-WEYFDNX30"
	// GOOG_API_KEY_2    string = "AIzaSyAO_FJ2SlqU8Q4STEHLGCilw_Y9_11qcW8"
)

const (
	OTHER_INFO_SEPARATOR string = " • "
	EXPLICIT_ICON        rune   = '🅴'
)
